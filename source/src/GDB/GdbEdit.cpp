...
bool GdbFile::RemoveRecord(uint32_t rec_hash)
{
    const int idx = FindRecord(rec_hash);
    if (idx < 0) return false;

    records_.erase(records_.begin() + idx);
    record_index_by_hash_.erase(rec_hash);
    for (size_t i = size_t(idx); i < records_.size(); ++i) {
        record_index_by_hash_[records_[i].hash] = int(i);
    }
    name_map_.erase(
        std::remove_if(name_map_.begin(), name_map_.end(),
                       [rec_hash](const auto& mapping) {
                           return mapping.second == rec_hash;
                       }),
        name_map_.end());
    return true;
}
...
uint32_t GdbFile::ResolveNamedRecord(const std::string& name) const
{
    const uint32_t key = fnv1(name);
    const auto it = std::lower_bound(
        name_map_.begin(), name_map_.end(), key,
        [](const std::pair<uint32_t, uint32_t>& mapping, uint32_t value) {
            return mapping.first < value;
        });
    return it != name_map_.end() && it->first == key ? it->second : 0;
}
