...
bool RebuildWithReplacedEntries(const std::string& bnk_path,
                                ...
                                std::string& err) {
    if (repls.empty()) { err = "no replacements given"; return false; }
    std::error_code ec;
    ...
    auto find_repl = [&](size_t i) -> const EntryReplacement* {
        for (const auto& r : repls) {
            if ((size_t)r.file_index == i) return &r;
        }
        return nullptr;
    };
    for (const auto& r : repls) {
        if (r.file_index < 0 || r.file_index >= (int)entries.size()) {
            err = "entry index out of range";
            return false;
        }
        if (entries[(size_t)r.file_index].csize != 0) {
    ...
    for (const auto& r : repls) {
        new_disk[(size_t)r.file_index] = (uint32_t)r.payload.size();
    }
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        const EntryReplacement* r = find_repl(i);
        put32(new_table, r ? (uint32_t)r->payload.size() : e.usize);
        if (compress_flag == 1) {
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        if (const EntryReplacement* r = find_repl(i)) {
            std::memcpy(dst, r->payload.data(), r->payload.size());
        } else {
