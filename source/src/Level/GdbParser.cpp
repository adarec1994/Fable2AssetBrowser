                               ...
                               uint32_t& out_hash)
{
    ...
    if (!view.findField(record, field_hash, 0xFF,
                                   ...
                                   uint32_t& out_hash)
{
    ...
    if (TryReadStaticMeshModelPathHash(view, record, out_hash)) return true;
    return TryReadStaticMultipleModelPathHash(view, record, out_hash);
}
    ...
    const std::vector<std::pair<uint32_t, std::string>>& hash_to_name)
{
    ...
    for (size_t i = 0; i + 1 < var_offs.size(); ++i) {
        ...
        if (view.ok && inst_hash != 0) {
            ...
            size_t parent_slot = 0;
            if (view.lookup(inst_hash, direct)) {
                if (view.findLocal(direct, kHashParent, 6,
                                   parent_slot, nullptr)) {
                    pl.parent_hash = ReadBeU32(bytes.data() + parent_slot);
                }
                if (!TryReadModelPathHashForRecord(view, direct,
                                                   pl.model_path_hash) &&
                    pl.parent_hash != 0) {
                    TryReadModelPathHashForHash(view, pl.parent_hash,
                                                pl.model_path_hash);
                }
            }
        }
