...
bool parse_mdl_geometry(const std::vector<unsigned char>& data, const MDLInfo& info, std::vector<MDLMeshGeom>& out){
    ...
    for(size_t mi=0; mi<info.MeshBuffers.size(); ++mi){
        ...
        if(mb.VertexCount==0 || mb.FaceCount==0 || mb.VertexOffset+(size_t)mb.VertexCount*vertex_stride>r.n || mb.FaceOffset+(size_t)mb.FaceCount*2>r.n){
            MDLMeshGeom g;
            if(meshIdx<info.Meshes.size() && !info.Meshes[meshIdx].Materials.empty())
                g.diffuse_tex_name=info.Meshes[meshIdx].Materials[0].TextureName;
            g.MeshIndex = meshIdx;
            g.SubMeshIndex = 0;
        ...
        if(hasFFFF){ build_triangles_from_strip(strip, all_indices); }
        else{
            size_t triCount=strip.size()/3; all_indices.resize(triCount*3);
            for(size_t t=0;t<triCount;t++){ all_indices[t*3+0]=strip[t*3+0]; all_indices[t*3+1]=strip[t*3+1]; all_indices[t*3+2]=strip[t*3+2]; }
        }

        std::vector<float> all_normals;
        compute_smooth_normals(mb.VertexCount, all_indices, all_positions, all_normals);

        if(mb.SubMeshes.empty() || mb.SubMeshCount <= 1){
            MDLMeshGeom g;
            g.positions = all_positions;
            g.normals = all_normals;
            g.uvs = all_uvs;
            g.indices = all_indices;
            ...
            for(size_t si=0; si<mb.SubMeshes.size(); ++si){
                ...
                uint8_t matIdx = sub.MaterialIndex;
                if(meshIdx<info.Meshes.size() && matIdx<info.Meshes[meshIdx].Materials.size())
                    g.diffuse_tex_name=info.Meshes[meshIdx].Materials[matIdx].TextureName;
                else if(meshIdx<info.Meshes.size() && !info.Meshes[meshIdx].Materials.empty())
                    g.diffuse_tex_name=info.Meshes[meshIdx].Materials[0].TextureName;

                uint32_t startIdx = sub.StartIndex;
                uint32_t idxCount = sub.IndexCount;

                if(startIdx + idxCount > all_indices.size()){
                    idxCount = (all_indices.size() > startIdx) ? (uint32_t)(all_indices.size() - startIdx) : 0;
                }
                ...
                std::vector<uint32_t> sub_indices(all_indices.begin() + startIdx, all_indices.begin() + startIdx + idxCount);
                ...
                for(uint32_t v=0; v<mb.VertexCount; ++v){
                    if(used_verts[v]){
                        ...
                        g.normals[nv*3+0] = all_normals[v*3+0];
                        g.normals[nv*3+1] = all_normals[v*3+1];
                        g.normals[nv*3+2] = all_normals[v*3+2];
