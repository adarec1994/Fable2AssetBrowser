...
static bool create_target(ID3D11Device* dev, ModelPreview& mp, int w, int h){
    ...
    D3D11_SAMPLER_DESC ssd{}; ssd.Filter=D3D11_FILTER_MIN_MAG_MIP_LINEAR; ssd.AddressU=ssd.AddressV=ssd.AddressW=D3D11_TEXTURE_ADDRESS_WRAP; ssd.MaxLOD=D3D11_FLOAT32_MAX;
...
void MP_Render(ID3D11Device* dev, ModelPreview& mp, float yaw, float pitch, float dist){
    ...
    cb.lightDir = XMFLOAT4(-0.4f, 0.85f, 0.45f, 0.0f);
...
bool MP_Build(const std::vector<MDLMeshGeom>& geoms, const MDLInfo& info, ModelPreview& mp) {
    ...
    for (size_t i = 0; i < geoms.size(); ++i) {
        ...
        bool hasA = false;
        if (!g.diffuse_tex_name.empty()) {
            m.tex_diffuse = load_tex_from_name(g.diffuse_tex_name, &hasA);
        }
...
void MP_Render(ModelPreview& mp, float yaw, float pitch, float dist) {
    ...
    float ex = mp.center[0] + r * cp * sy;
    float ey = mp.center[1] + r * sp;
    float ez = mp.center[2] + r * cp * cy;
    ...
    for (const auto& m : mp.meshes) {
        if (!m.vao || m.index_count == 0) continue;
        ...
        glBindTexture(GL_TEXTURE_2D, m.tex_diffuse ? m.tex_diffuse : mp.default_tex);
        glUniform1i(mp.tex_diffuse_loc, 0);

        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, m.tex_normal ? m.tex_normal : mp.default_tex);
        glUniform1i(mp.tex_normal_loc, 1);

        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, m.tex_specular ? m.tex_specular : mp.default_tex);
        glUniform1i(mp.tex_specular_loc, 2);

        glActiveTexture(GL_TEXTURE3);
        glBindTexture(GL_TEXTURE_2D, m.tex_unk ? m.tex_unk : mp.default_tex);
        glUniform1i(mp.tex_unk_loc, 3);

        glActiveTexture(GL_TEXTURE4);
        glBindTexture(GL_TEXTURE_2D, m.tex_tint ? m.tex_tint : mp.default_tex);
        glUniform1i(mp.tex_tint_loc, 4);
